...
library(mvtnorm)
library(xlsx)
setwd('C:/Users/u0118563/OneDrive - KU Leuven/Projecten/Gepaarde data/Grote simulatie/Datasets/check')
#Set seed
set.seed(123)

#Sample the treatment effect at baseline and on the evolution (slope)
treat_bl=rep(rnorm(1),mean=5,sd=2)
treat_sl=rep(rnorm(1),mean=10,sd=10)
...
effect_pair=rnorm(n_pairs,sd=2)
...
for(i in 1:100){
  ...
  #simulate the slope
  control_sl=rep(rnorm(1),mean=10,sd=10)
  #simulate the intercept
  control_bl=rnorm(1)
  
  #create the data
  data_cases_t1=treat_bl+control_bl+effect_pair+effects_subject[1:n_pairs,1]++rnorm(n=n_pairs,sd=2)
  data_cases_t2=treat_bl+control_bl+effect_pair+effects_subject[1:n_pairs,1]+(effects_subject[1:n_pairs,2]+treat_sl+control_sl)*1+rnorm(n=n_pairs,sd=2)
  data_cases_t3=treat_bl+control_bl+effect_pair+effects_subject[1:n_pairs,1]+(effects_subject[1:n_pairs,2]+treat_sl+control_sl)*2+rnorm(n=n_pairs,sd=2)
  data_cases_t4=treat_bl+control_bl+effect_pair+effects_subject[1:n_pairs,1]+(effects_subject[1:n_pairs,2]+treat_sl+control_sl)*3+rnorm(n=n_pairs,sd=2)
  data_cases_t5=treat_bl+control_bl+effect_pair+effects_subject[1:n_pairs,1]+(effects_subject[1:n_pairs,2]+treat_sl+control_sl)*4+rnorm(n=n_pairs,sd=2)
  ...
  data_controls_t2=control_bl+effect_pair+effects_subject[(n_pairs+1):(n_pairs*2),1]+(control_sl+effects_subject[(n_pairs+1):(n_pairs*2),2])*1+rnorm(n=n_pairs,sd=2)
  data_controls_t3=control_bl+effect_pair+effects_subject[(n_pairs+1):(n_pairs*2),1]+(control_sl+effects_subject[(n_pairs+1):(n_pairs*2),2])*2+rnorm(n=n_pairs,sd=2)
  data_controls_t4=control_bl+effect_pair+effects_subject[(n_pairs+1):(n_pairs*2),1]+(control_sl+effects_subject[(n_pairs+1):(n_pairs*2),2])*3+rnorm(n=n_pairs,sd=2)
  data_controls_t5=control_bl+effect_pair+effects_subject[(n_pairs+1):(n_pairs*2),1]+(control_sl+effects_subject[(n_pairs+1):(n_pairs*2),2])*4+rnorm(n=n_pairs,sd=2)
